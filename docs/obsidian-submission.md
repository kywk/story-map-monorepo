...
The GitHub tag must exactly equal the plugin version, **`0.1.0`**, without `v` or
...
the matching plain-version GitHub tag with fresh assets. Update `versions.json` when
minimum compatibility changes. Do not reuse or replace an already published version.
